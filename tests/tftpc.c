...
/*set the address values for the server */
  ...
  if (syscall(SYS_sendto_rtnet, sock, buf, len, 0, (struct sockaddr *) &server, server_len) !=
      len)
    {
...
/* The following 'for' loop is used to recieve/timeout ACKs */
  for (j = 0; j < RETRIES - 2; j++)
	...
	{
	  n = syscall(SYS_recvfrom_rtnet, sock, recvbuf, sizeof (recvbuf), MSG_DONTWAIT,
	...
	{			/*changed client to server here */
	  ...
	  if (tid != ntohs (server.sin_port))	/* checks to ensure get from the correct TID */
		...
		printf
	      ...
	      if (syscall(SYS_sendto_rtnet, sock, buf, len, 0, (struct sockaddr *) &server, sizeof (server)) != len)	/* send the data packet */
		{
		  printf
...
/* The ack sending 'for' loop ends here */
  ...
  while (1)			/* our break statement will escape us when we are done */
	...
	printf ("Client: Sending packet # %04d (length: %d file chunk: %d)\n",
      ...
      if (syscall(SYS_sendto_rtnet, sock, packetbuf[bcount], len, 0, (struct sockaddr *) &server,
	   sizeof (server)) != len)
	{
	  if (debug)
...
/* The following 'for' loop is used to recieve/timeout ACKs */
	  for (j = 0; j < RETRIES; j++)
		...
		{
		  n =
		    syscall(SYS_recvfrom_rtnet, sock, recvbuf, sizeof (recvbuf), MSG_DONTWAIT,
			      (struct sockaddr *) &ack,
		...
		{		/* checks to ensure send to ip is same from ACK IP */
		  ...
		  if (tid != ntohs (server.sin_port))	/* checks to ensure get from the correct TID */
			...
			printf
		      ...
		      len = err_packet (5, err_msg[5], buf);
		      /* send the data packet */
		      if (syscall(SYS_sendto_rtnet, sock, buf, len, 0, (struct sockaddr *) &server,
			   sizeof (server)) != len)
			{
			  printf
			    ("Client: Mismatch in number of sent bytes while trying to send mode error packet\n");
...
/* this formatting code is just like the code in the main function */
	      ...
	      for (i = 0; i <= bcount; i++)
		{
		  if (syscall(SYS_sendto_rtnet, sock, packetbuf[i], len, 0, (struct sockaddr *) &server, sizeof (server)) != len)	/* resend the data packet */
		    {
		      if (debug)
			printf ("Client: Mismatch in number of sent bytes\n");
		      return;
      ...
      else if (debug)
	{
	  printf
	    ("Client: Not attempting to recieve ack. Not required. count: %d\n",
	     count);
	  n = syscall(SYS_recvfrom_rtnet, sock, recvbuf, sizeof (recvbuf), MSG_DONTWAIT, (struct sockaddr *) &ack, (socklen_t *) & server_len);	/* just do a quick check incase the remote host is trying with ackfreq = 1 */
	  /*if (debug)
	     ip_port(ack); */
	}
...
/* zero the buffer before we begin */
  ...
  do
      ...
      if (n != (datasize + 4))	/* remember if our datasize is less than a full packet this was the last packet to be received */
	{
	  ...
	  if (syscall(SYS_sendto_rtnet, sock, ackbuf, len, 0, (struct sockaddr *) &server,
	       sizeof (server)) != len)
	    {
	      if (debug)
      ...
      for (j = 0; j < RETRIES; j++)	/* this allows us to loop until we either break out by getting the correct ack OR time out because we've looped more than RETRIES times */
	{
	  ...
	  for (i = 0; errno == EAGAIN && i <= TIMEOUT && n < 0; i++)	/* this for loop will just keep checking the non-blocking socket until timeout */
	    {
	      ...
	      n =
		syscall(SYS_recvfrom_rtnet, sock, packetbuf, sizeof (packetbuf) - 1,
	  ...
	  if (n < 0 && errno != EAGAIN)	/* this will be true when there is an error that isn't the WOULD BLOCK error */
	      ...
	      if (tid != ntohs (server.sin_port))	/* checks to ensure get from the correct TID */
		{
		  ...
		  if (syscall(SYS_sendto_rtnet, sock, packetbuf, len, 0, (struct sockaddr *) &server, sizeof (server)) != len)	/* send the data packet */
		    {
		      printf

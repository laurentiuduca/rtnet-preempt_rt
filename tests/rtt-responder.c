...
#else				/* with_recvfrom */
		ret =
		    syscall(SYS_recvfrom_rtnet, sock, &packet, BUFSIZE, 0,
...
#else				/* with_recvfrom */
		syscall(SYS_sendto_rtnet, sock, &packet, ret, 0,
			(struct sockaddr *)&dest_addr, addrlen);
#endif
#if 0
		printf
		    ("ret=%d, packet.tx_date.tv_sec=%ld packet.tx_date.tv_nsec=%ld sent=%d\n",
		     ret, packet.tx_date.tv_sec, packet.tx_date.tv_nsec,
		     reply_size ? : ret);
#endif
	}

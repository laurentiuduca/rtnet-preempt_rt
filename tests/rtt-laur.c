...
#ifdef vanilla
	if ((ret =
	     sendto(sock, str, strlen(str), 0, (struct sockaddr *)&dest_addr,
		    sizeof(struct sockaddr_in))) < 0) {
#elif with_recvmsg
	if ((ret = syscall(SYS_sendmsg_rtnet, sock, &msg, 0)) < 0) {
#else				/* with_recvfrom */
	if ((ret =
	     syscall(SYS_sendto_rtnet, sock, str, strlen(str), 0,
...
#define POLL_INIFINITE_TIMEOUT -1
...
#else
	if ((ret =
	     syscall(SYS_poll_rtnet, &pfd, 1, POLL_INIFINITE_TIMEOUT)) < 0) {
#endif
		perror("poll");
		printf("poll error: %d\n", errno);
		return NULL;
...
#else				/* with_recvfrom */
	ret =
	    syscall(SYS_recvfrom_rtnet, sock, recvbuf, BUFSIZE, 0,
	...
	if (ret <= 0) {
		...
		printf("recvmsg done: iov=%s ret=%d IP=%s\n",
		       ...
		       inet_ntoa(((struct sockaddr_in *)msg.msg_name)->
				 sin_addr));
#elif with_recvmsg
...
#else
	if ((sock =
	     syscall(SYS_socket_rtnet, AF_INET, SOCK_DGRAM, IPPROTO_UDP)) < 0) {
#endif
...
#ifdef vanilla
		if (bind
		    (sock, (struct sockaddr *)&local_addr,
		     sizeof(local_addr)) < 0) {
#else
		if (syscall
		    (SYS_bind_rtnet, sock, (struct sockaddr *)&local_addr,
		     sizeof(local_addr)) < 0) {
#endif

...
int main()
{
	...
	if ((ioctl(sock_raw, SIOCGIFHWADDR, &ifreq_c)) < 0) {
		...
		printf("mac address: %x %x %x %x %x %x\n",
		       ifreq_c.ifr_hwaddr.sa_data[0],
		       ifreq_c.ifr_hwaddr.sa_data[1],
		       ifreq_c.ifr_hwaddr.sa_data[2],
		       ifreq_c.ifr_hwaddr.sa_data[3],
		       ifreq_c.ifr_hwaddr.sa_data[4],
		       ifreq_c.ifr_hwaddr.sa_data[5]);
	}
	...
	if (ioctl(sock_raw, SIOCGIFADDR, &ifreq_ip) < 0) {
		...
		printf("ip src: %s\n",
		       inet_ntoa((((struct sockaddr_in *)&(ifreq_ip.ifr_addr))->
				  sin_addr)));
	}
	...
	iph->saddr =
	    inet_addr(inet_ntoa
		      ((((struct sockaddr_in *)&(ifreq_ip.ifr_addr))->
			sin_addr)));
	...
	iph->check =
	    checksum((unsigned short *)(sendbuff + sizeof(struct ethhdr)),
...
#else
	send_len =
	    syscall(SYS_sendto_rtnet, sock_raw, sendbuff, 64, 0,

...
#ifdef vanilla
		if (sendto
		    (sock, &tx_packet, sizeof(time_struct_64), 0,
...
#else				/* with_recvfrom */
		if (syscall
		    (SYS_sendto_rtnet, sock, &tx_packet, sizeof(time_struct_64),
...
#else				/* with_recvfrom */
	ret =
	    syscall(SYS_recvfrom_rtnet, sock, &rx_packet, BUFSIZE, 0,
		    (struct sockaddr *)&rx_addr, &rx_addrlen);
#endif
	if (ret <= 0) {
